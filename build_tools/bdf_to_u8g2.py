...
[p.unlink() for p in out_source_dir.glob("*_font.c*")]

header_path = out_source_dir / "u8g2_vexed_everyday_fonts.h"
...
for bdf_path in bdf_paths:
    ...
    out_path = out_source_dir / f"{name}_font.c"
    tmp = out_path.with_suffix(".c.tmp")
    logging.info(f"\n🔠 Creating: {out_path}")
    subproc.run(bdfconv, "-b0", "-f1", f"-n{name}_font", f"-o{tmp}", bdf_path)
...
header_path.write_text(textwrap.dedent(f"""
    ...
    {"\n    ".join(f"extern const uint8_t {n}_font[];" for n in names)}
...
all_fonts_path = out_source_dir / "all_everyday_fonts.c"
...
all_fonts_path.write_text(textwrap.dedent(f"""
    ...
    const struct everyday_entry all_everyday_fonts[{len(names) + 1}] = {{
      {"\n      ".join(f"{{ {json.dumps(n)}, {n}_font }}," for n in names)}
      {{ NULL, NULL }}
    }};
""").lstrip("\n"))
